    ...
    <div className="bg-[#F6F9FE] min-h-screen">
      <div className="max-w-[1280px] mx-auto py-8 px-4 md:px-6 lg:px-8 rounded-lg shadow-sm">
        ...
        <div className="border-1 border-border_primary bg-white p-4 rounded-lg">
          <div className="border-b mb-6">
            <div className="flex -mb-px">
              <button
                className={`mr-6 py-2 text-sm font-medium border-b-2 ${activeTab === "myPosts" ? "text-blue-600 border-blue-600" : "text-gray-500 border-transparent hover:text-gray-700"}`}
                onClick={() => setActiveTab("myPosts")}
              >
                {jobBoardLanguageData?.tab_all_jobs}
              </button>
              <button
                className={`py-2 text-sm font-medium border-b-2 ${activeTab === "closedPosts" ? "text-blue-600 border-blue-600" : "text-gray-500 border-transparent hover:text-gray-700"}`}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[120px] visible">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {jobPosts.length > 0 ? (
                    jobPosts.map((job) => (
                      <tr key={job.id} onClick={() => route.push(`/job-board/${job.id}`)} className="hover:bg-gray-50 cursor-pointer">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-start">
                            <div className="mr-2 mt-1">
                              <svg className="h-5 w-5 text-gray-400" viewBox="0 0 24 24" fill="none">
                                <path d="M9 12h6m-3-3v6M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                              </svg>
                            </div>
                            <div>
                              <Link href={`/job-board/${job.id}`} className="hover:text-blue-600 font-medium text-[14px] text-text_primary font-sans">
                                {job.job_title}
                              </Link>
                            </div>
                          </div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{job.working_from}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                          {parseFloat(job.budget).toLocaleString()}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 visible">{formatDate(job.created_at)}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 visible">{formatDate(job.deadline) || "-"}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
        ...
        <div className="mt-12 h-[148px] bg-[#D0E1FB] rounded-lg overflow-hidden flex justify-center items-center">
          <Image src={ProfileImage.job_board} alt="Job Board" className="w-auto h-full object-contain" />
        </div>
      </div>
    </div>
  );
