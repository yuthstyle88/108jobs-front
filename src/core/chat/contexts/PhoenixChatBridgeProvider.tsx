...
import { ResendManager, type ChatStorePort } from "@/core/chat/services/ResendManager";
...
export const PhoenixChatBridgeProvider: React.FC<WebSocketProviderProps> = ({ children, roomId }) => {
  ...
  useEffect(() => {
    ...
    const sender = new PhoenixSenderAdapter(ch, (clientId) => { servicesRef.current.resend?.onSendFailure(clientId); });
    ...
    const onOpen = () => {
      ...
      // ปลุก resend ทุกห้องเมื่อกลับมา online
      void resend.flushAll();
    };
    const onClose = () => {
  ...
  useEffect(() => {
    ...
    if (ch && typeof ch.on === "function") {
      try { ch.on("chat:message", toAck); } catch {}
      try { ch.on("forward", toAck); } catch {}
      return () => {
        try { ch.off?.("chat:message", toAck); } catch {}
        try { ch.off?.("forward", toAck); } catch {}
      };
    } else {
      try { ws.on?.("chat:message", toAck); } catch {}
      try { ws.on?.("forward", toAck); } catch {}
      return () => {
        try { ws.off?.("chat:message", toAck); } catch {}
        try { ws.off?.("forward", toAck); } catch {}
      };
    }
  }, [ws, roomId]);
