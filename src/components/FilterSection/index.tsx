...
const FilterSection = ({ language, onFilterChange, currentFilters }: Props) => {
  ...
  return (
    <>
      {activeFilterCount > 0 ? (
        <div
          className={`${activeFilterCount > 0 ? "bg-secondary text-third border-third hover:bg-[#dae5f7]" : "hover:bg-[#F6F7F8] border-borderPrimary"} filter-button px-4 py-2 rounded-md flex items-center gap-2 text-text_primary cursor-pointer hover:bg-gray-100 transition`}
        ...
        <div
          onClick={() => setIsFilterOpen(true)}
          className="filter-button border border-borderPrimary px-4 py-2 rounded-md flex items-center gap-2 text-text_primary cursor-pointer hover:bg-gray-100 transition"
        >
