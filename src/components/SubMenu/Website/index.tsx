...
const Website: React.FC = () => {
    ...
    const webService = catalogData?.communities.find(
        (catalog) => catalog.community.name.toLowerCase() === "web programming"
    );

    if (isLoading) return <Loading/>;

    if (!webService) return <ErrorState/>;
    ...
    return (
        <div
            ...
            <Link prefetch={false} href={`/categories/${webService.community.id}`}
                  className="px-2 text-[#485261] whitespace-nowrap sm:whitespace-normal">{webService.community.name}</Link>
            <div
                ...
                <CategoryList
                    key={webService.community.id}
                    title={webService.community.name}
                    items={webService.children}
                />
            </div>
        </div>
    );
