...
const Lifestyle: React.FC = () => {
    ...
    const operateService = catalogData?.communities.find(
        (catalog) => catalog.community.name.toLowerCase() === "operate"
    );

    if (!operateService) return null;
    ...
    return (
        <div
            ...
            <Link prefetch={false} href={`/categories/${operateService.community.id}`}
                  className="px-2 text-[#485261] whitespace-nowrap sm:whitespace-normal">{operateService.community.name}</Link>
            <div
                className="grid grid-cols-[1fr_1fr] max-h-sub-menu overflow-auto gap-y-4 absolute right-0 w-[430px] opacity-0 scale-y-0 origin-top top-[3.5rem] shadow-sub-menu px-[1rem] py-[1rem] text-[rgba(43,50,59,.95)] z-50 bg-white border-t-[1px] border-t-secondary border-b-2 border-b-third group-hover:opacity-100 group-hover:scale-y-100 transition-all duration-300">
                <CategoryList
                    key={operateService.community.id}
                    title={operateService.community.name}
                    items={operateService.children}
                />
            </div>
        </div>
    );
