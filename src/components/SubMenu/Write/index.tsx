...
const Write: React.FC = () => {
    ...
    const writeService = catalogData?.communities.find(
        (catalog) => catalog.community.name.toLowerCase() === "writing & translation"
    );

    if (isLoading) return <Loading/>;

    if (!writeService) return <ErrorState/>;
  ...
  return (
    <div
      ...
      <Link prefetch={false} href={`/categories/${writeService.community.id}`}
            className="px-2 text-[#485261] whitespace-nowrap sm:whitespace-normal">{writeService.community.name}</Link>
      <div
        className="grid grid-cols-[1fr_1fr_1fr] gap-y-4 absolute left-0 w-[630px] right-0 opacity-0 scale-y-0 origin-top top-[3.5rem] shadow-sub-menu px-[1rem] py-[1rem] text-[rgba(43,50,59,.95)] z-50 bg-white border-t-[1px] border-t-secondary border-b-2 border-b-third group-hover:opacity-100 group-hover:scale-y-100 transition-all duration-300">
          <CategoryList
            key={writeService.community.id}
            title={writeService.community.name}
            items={writeService.children}
          />
      </div>
    </div>
  );
