...
const GraphicDesign = () => {
    ...
    const graphicDesignService = catalogData?.communities.find(
        (catalog) => catalog.community.name.toLowerCase() === "graphic design"
    );

    if (!graphicDesignService) return null;
    ...
    return (
        <div
            ...
            <Link prefetch={false} href={`/categories/${graphicDesignService.community.id}`}
                  className="px-2 text-[#485261] whitespace-nowrap sm:whitespace-normal">{graphicDesignService.community.name}</Link>
            <div
                ...
                <CategoryList
                    key={graphicDesignService.community.id}
                    title={graphicDesignService.community.name}
                    items={graphicDesignService.children}
                />
            </div>
        </div>
    );
