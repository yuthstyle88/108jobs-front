    ...
    <div id="package">
      <div className="grid grid-cols-[1fr] gap-y-6">
        <h2 className="text-[1.25rem] text-third font-medium">
          {language?.package_details} {data.title}
        </h2>
        {data.packages.map((pkg) => (
          <div
            key={pkg.id}
            className="px-6 pt-6 pb-4 border-[0.0625rem] border-border_primary rounded-[0.25rem]"
          >
            ...
            <div className="mt-2 text-[0.875rem] text-text_secondary flex flex-row gap-2 items-center font-sans">
              ...
              />
              <p>ระยะเวลาในการทำงาน {pkg.execution_time} วัน</p>
            </div>
