...
export type UseWorkflowActionsDeps = {
  ...
  startWorkflow: (form: { postId: number | string; seqNumber: number; roomId: string }) => Promise<any>;
...
export const useWorkflowActions = (deps: UseWorkflowActionsDeps) => {
  ...
  const quotationSubmit = useCallback(async (data: any) => {
    ...
    try {
      ...
      const sentId = await sendStructuredMessage(sendMessage, roomId, payload, { senderId: Number(localUser?.id) || 0, previewText: readable });
  ...
  const approveQuotation = useCallback(async () => {
    try {
      ...
      const form: ApproveQuotationForm = { seqNumber, billingId, walletId: (roomData as any)?.person?.walletId, workflowId } as any;
  ...
  const startWork = useCallback(async () => {
    try {
      ...
      const seqNumber = getLatestProposedQuoteSeq(messages as any, 1);
      const form: any = { seqNumber, workflowId, workDescription: t('profileChat.startWorkMsg') || 'Freelancer started work.' };
